...
class CROMA(nn.Module):
    ...
    def forward(self, imgs, radar_mask_info, optical_mask_info, rank=0, world_size=1):
        ...
        return contrastive_loss, mae_loss, optical_GAP, radar_GAP

    @torch.no_grad()
    def encode_s2(self, imgs: torch.Tensor, return_tokens: bool = False, use_gap_ffn: bool = True):
        """
        imgs: [B, 12, H, W], first 10 channels are S2
        returns: pooled [B, D] if return_tokens is False,
                 else a tuple (pooled [B, D], tokens [B, L, D])
        """
        self.eval()
        optical_imgs = imgs[:, :10, ...]
        alibi = self.attn_bias.to(optical_imgs.device)
        tokens = self.optical_encoder(imgs=optical_imgs, attn_bias=alibi, mask_info=None)       # [B, L, D]
        pooled = tokens.mean(dim=1)                                                             # [B, D]
        if use_gap_ffn:
            pooled = self.GAP_FFN_optical(pooled)                                               # [B, D]
        return (pooled, tokens) if return_tokens else pooled

    @torch.no_grad()
    def encode_s1(self, imgs: torch.Tensor, return_tokens: bool = False, use_gap_ffn: bool = True):
        """
        imgs: [B, 12, H, W], last 2 channels are S1
        returns: pooled [B, D] if return_tokens is False,
                 else a tuple (pooled [B, D], tokens [B, L, D])
        """
        self.eval()
        ...
        tokens = self.radar_encoder(imgs=radar_imgs, attn_bias=alibi, mask_info=None)           # [B, L, D]
        pooled = tokens.mean(dim=1)                                                             # [B, D]
        if use_gap_ffn:
            pooled = self.GAP_FFN_radar(pooled)                                                 # [B, D]
        return (pooled, tokens) if return_tokens else pooled
...
def gather_features(features: torch.Tensor, world_size: int) -> torch.Tensor:
...
class DecoderMAE(nn.Module):
    ...
    def forward(self, x, mask_info_radar, mask_info_optical, target):
        ...
        pred = rearrange(x, 'b (h w) (c i j) -> b c (h i) (w j)', c=12, i=8, j=8, h=15, w=15)
        pred_optical = rearrange(pred[:, :10, :, :], 'b c (h i) (w j) -> b (h w) (c i j)', c=10, i=8, j=8)
        pred_radar = rearrange(pred[:, 10:, :, :], 'b c (h i) (w j) -> b (h w) (c i j)', c=2, i=8, j=8)
        ...
        target = rearrange(target, 'b (h w) (c i j) -> b c (h i) (w j)', c=12, i=8, j=8, h=15, w=15)
        target_optical = rearrange(target[:, :10, :, :], 'b c (h i) (w j) -> b (h w) (c i j)', c=10, i=8, j=8)
        target_radar = rearrange(target[:, 10:, :, :], 'b c (h i) (w j) -> b (h w) (c i j)', c=2, i=8, j=8)
